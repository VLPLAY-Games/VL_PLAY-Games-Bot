import os, uuid, imageio, io, speech_recognition as sr, moviepy.editor as mp, pytesseract, random, hashlib, wikipediaapi, requests, sqlite3
...
inlineKeyboardStart.add(
    ...
    InlineKeyboardButton("Discord Server" if user_language == "en"  else "Discord сервер", callback_data="discord"),
    InlineKeyboardButton("Youtube channel" if user_language == "en"  else "Youtube канал", callback_data="youtube"),
    InlineKeyboardButton("About Bot" if user_language == "en"  else "Информация о боте", callback_data="about_bot"),
    InlineKeyboardButton("Commands" if user_language == "en"  else "Команды", callback_data="commands"),
)
...
inlineKeyboardGames.add(
    ...
    InlineKeyboardButton("Game engine" if user_language == "en"  else "Игровой движок", callback_data="engine"),
)
inlineKeyboardQlake = InlineKeyboardMarkup()
inlineKeyboardQlake.add(
    InlineKeyboardButton("Release Date" if user_language == "en"  else "Дата выхода", callback_data="qlake_date"),
    InlineKeyboardButton("About Qlake" if user_language == "en"  else "Обо Qlake", callback_data="qlake_about"),
    InlineKeyboardButton("Link" if user_language == "en"  else "Ссылка", callback_data="qlake_link"),
    InlineKeyboardButton("System Requirements" if user_language == "en"  else "Системные требования", callback_data="qlake_sr"),
    InlineKeyboardButton("Trailer" if user_language == "en"  else "Трейлер", callback_data="qlake_trailer"),
)
inlineKeyboardSD = InlineKeyboardMarkup()
inlineKeyboardSD.add(
    InlineKeyboardButton("Release Date" if user_language == "en"  else "Дата выхода", callback_data="sd_date"),
    InlineKeyboardButton("About Silent Darkness" if user_language == "en"  else "Обо Silent Darkness", callback_data="sd_about"),
    InlineKeyboardButton("Link" if user_language == "en"  else "Ссылка", callback_data="sd_link"),
    InlineKeyboardButton("System Requirements" if user_language == "en"  else "Системные требования", callback_data="sd_sr"),
    InlineKeyboardButton("Trailer" if user_language == "en"  else "Трейлер", callback_data="sd_trailer"),
)
...
async def callback_message(call: types.CallbackQuery):
    ...
    # Silent Darkness
        ...
        await bot.send_message(call.message.chat.id, "The game is under development. Game release date unknown" if user_language == "en" else "Игра находится в стадии разработки. Дата выхода игры неизвестна")
    elif call.data == "sd_about":
        await bot.send_message(call.message.chat.id, "The game is under development. Game description unknown" if user_language == "en" else "Игра находится в стадии разработки. Описание игры неизвестно")
    elif call.data == "sd_link":
        await bot.send_message(call.message.chat.id, "The game is under development. Link unknown" if user_language == "en" else "Игра находится в стадии разработки. Ссылка неизвестна")
    elif call.data == "sd_sr":
        await bot.send_message(call.message.chat.id, "The game is under development. System Requirements unknown" if user_language == "en" else "Игра находится в стадии разработки. Системные требования неизвестны")
    ...
    # Hash
        ...
        global method_hash
        method_hash = 'md5'
        await bot.send_message(call.message.chat.id, "Send file:" if user_language == "en" else "Отправьте файл")
    ...
    # Wikipedia
        ...
        global wiki_lang
        wiki_lang = 'ru'
        await bot.send_message(call.message.chat.id, "Submit a text:" if user_language == "en" else "Отправьте текст")
    ...
    # Язык
        ...
        user_language = 'ru'
        c.execute('UPDATE users SET language = ? WHERE user_id = ?', ('ru', call.message.chat.id))
        conn.commit()
        await bot.send_message(call.message.chat.id, "Выбранный язык: Русский")

    elif call.data == "lang_en":
        user_language = 'en'
        c.execute('UPDATE users SET language = ? WHERE user_id = ?', ('en', call.message.chat.id))
        conn.commit()
...
async def start(msg: types.Message):
    ...
    c.execute('SELECT * FROM users WHERE user_id = ?', (msg.chat.id,))
    user_record = c.fetchone()

    if not user_record:
        # If the user is not in the database, insert a new record with the default language
        c.execute('INSERT INTO users (user_id, language) VALUES (?, ?)', (msg.chat.id, 'en'))
        conn.commit()
        user_language = 'en'  # Set default language for new users
    else:
        # If the user is in the database, retrieve their language
        user_language = user_record[1]

    await bot.send_message(msg.chat.id, "How can I help you?" if user_language == "en" else "Могу ли я вам помочь?", reply_markup=inlineKeyboardStart)
...
async def bot_languge_select(message: types.Message):
    await message.reply('Select language:' if user_language == "en" else "Выберите язык:", reply_markup=inlineKeyboardLanguage)
...
async def download_youtube_video_command(message: types.Message):
    await bot.send_message(message.chat.id, 'Enter YouTube video link.' if user_language == "en" else "Введите ссылку на видео YouTube")
...
async def handle_youtube_video(message: types.Message, state: FSMContext):
    try:
        await message.reply("Downloading video... \nThis may take a few minutes." if user_language == "en" else "Скачивание видео... \nЭто может занять несколько минут.")
        url = message.text
        yt = YouTube(url)
        ...
        video_file.close()
        os.remove(filename)
        await message.reply("Downloading complete." if user_language == "en" else "Скачивание завершено.")
    except Exception as e:
        await message.reply('An error occurred while downloading video from YouTube.' if user_language == "en" else "Произошла ошибка при загрузке видео с YouTube.")
...
async def convert_mp4_to_mp3_command(message: types.Message):
    await message.reply("Please send an MP4 video file to convert to MP3." if user_language == "en" else "Пожалуйста, отправьте видеофайл MP4 для конвертации в MP3.")
...
async def handle_mp4_to_mp3(message: types.Message, state: FSMContext):
    try:
        if message.video and message.video.mime_type == 'video/mp4':
            await message.reply('Converting video to audio... \nThis may take a few minutes.' if user_language == "en" else "Преобразование видео в аудио... \nЭто может занять несколько минут.")
            ...
            await message.reply('Conversion complete.' if user_language == "en" else "Преобразование завершено.")
        else:
            await message.reply('Please send an MP4 video file.' if user_language == "en" else "Пожалуйста, пришлите видеофайл в формате MP4.")
    except Exception as e:
        await message.reply('An error occurred during conversion.' if user_language == "en" else "Во время конвертации произошла ошибка.")
        report = open("report.txt", "a+")
        report.write("\n[" + str(datetime.now(timezone.utc)) + "] Server WARNING error: " + str(e) + version)
        report.close()
    await state.finish()
...
async def convert_mp3_to_wav_command(message: types.Message):
    await message.reply("Please send an MP3 audio file to convert to WAV." if user_language == "en" else "Пожалуйста, отправьте аудиофайл MP3 для конвертации в WAV.")
...
async def handle_mp3_to_wav(message: types.Message, state: FSMContext):
    try:
        if message.audio and message.audio.mime_type == 'audio/mpeg':
            await message.reply('Converting audio to WAV... \nThis may take a few minutes.' if user_language == "en" else "Преобразование аудио в WAV... \nЭто может занять несколько минут.")
            await bot.send_chat_action(chat_id=message.chat.id, action="upload_audio")
            ...
            await message.reply('Please send an MP3 audio file.' if user_language == "en" else "Пожалуйста, пришлите видеофайл в формате MP3.")
    except Exception as e:
        await message.reply('An error occurred during conversion.' if user_language == "en" else "Во время конвертации произошла ошибка.")
        report = open("report.txt", "a+")
...
async def convert_mp4_to_gif_command(message: types.Message):
    await message.reply("Please send an MP4 video file to convert to GIF." if user_language == "en" else "Пожалуйста, отправьте видеофайл MP4 для конвертации в GIF.")
...
async def handle_mp4_to_gif(message: types.Message, state: FSMContext):
    try:
        if message.video and message.video.mime_type == 'video/mp4':
            await message.reply('Converting video to GIF... \nThis may take a few minutes.' if user_language == "en" else "Преобразование видео в GIF... \nЭто может занять несколько минут.")
            await bot.send_chat_action(chat_id=message.chat.id, action="upload_video")
            file_info = await bot.get_file(message.video.file_id)
            downloaded_file = await bot.download_file(file_info.file_path)
        ...
        await message.reply('Error occurred during conversion.' if user_language == "en" else "Во время конвертации произошла ошибка.")
        report = open("report.txt", "a+")
        report.write("\n[" + str(datetime.now(timezone.utc)) + "] Server WARNING error: " + str(e))
        report.close()
    await state.finish()
...
async def convert_png_to_jpeg_command(message: types.Message):
    await message.reply("Please send a PNG image to convert to JPEG." if user_language == "en" else "Пожалуйста, отправьте картинку PNG для конвертации в JPEG.")
...
async def handle_png_to_jpeg(message: types.Message, state: FSMContext):
    try:
        if message.photo:
            await message.reply('Converting image to JPEG... \nThis may take a few minutes.' if user_language == "en" else "Преобразование картинки в JPEG... \nЭто может занять несколько минут.")
            ...
            await message.reply('Please send a PNG image.' if user_language == "en" else "Пожалуйста, пришлите картинку в формате PNG.")
    except Exception as e:
        await message.reply('An error occurred during conversion.' if user_language == "en" else "Во время конвертации произошла ошибка.")
        report = open("report.txt", "a+")
        report.write("\n[" + str(datetime.now(timezone.utc)) + "] Server WARNING error: " + str(e))
...
async def audio_recognition_en(message: types.Message):
    await bot.send_message(message.chat.id, 'Send a mp3 audio file.' if user_language == "en" else "Отправьте аудиофайл MP3.")
...
async def handle_audio_recognition_en(message: types.Message, state: FSMContext):
    try:
        if message.audio and message.audio.mime_type == "audio/mpeg":
            await message.reply("Audio conversion... \nThis may take a few minutes." if user_language == "en" else "Преобразование аудио... \nЭто может занять несколько минут.")
            ...
            await message.reply('Recognition complete.' if user_language == "en" else "Распознавание завершено.")
        else:
            await message.reply("Please send an MP3 file." if user_language == "en" else "Пожалуйста, пришлите аудиофайл в формате MP3.")
    except Exception as e:
        await message.reply("An error occurred during audio conversion." if user_language == "en" else "Во время конвертации аудио произошла ошибка.")
        report = open("report.txt", "a+")
        report.write("\n[" + str(datetime.now(timezone.utc)) + "] Server WARNING error: " + str(e) + version)
        report.close()
    await state.finish()
...
async def handle_audio_recognition_rus(message: types.Message, state: FSMContext):
    try:
        if message.audio and message.audio.mime_type == "audio/mpeg":
            ...
            await message.reply("Please send a MP3 file." if user_language == "en" else "Пожалуйста, пришлите аудиофайл в формате MP3.")
    except Exception as e:
        await message.reply("A error occurred while Audio conversion." if user_language == "en" else "Во время конвертации аудио произошла ошибка.")
        report = open("report.txt", "a+")
        report.write("\n[" + str(datetime.now(timezone.utc)) + "] Server WARNING error: " + str(e) + version)
        report.close()
    await state.finish()
...
async def convert_to_wav(message, file_path):
    ...
    await bot.send_message(message.chat.id, "Audio conversion completed. Text recognising..." if user_language == "en" else "Преобразование аудио завершено. Распознавание текста...")
...
async def convert_mov_to_mp4_command(message: types.Message):
    await message.reply('Send a MOV video file.' if user_language == "en" else "Отправьте видеофайл MOV.")
...
async def handle_convert_mov_to_mp4(message: types.Message, state: FSMContext):
    try:
        if message.video and message.video.mime_type == "video/quicktime":
            await message.reply("Converting video... \nThis may take a few minutes." if user_language == "en" else "Преобразование видео... \nЭто может занять несколько минут.")
            await bot.send_chat_action(chat_id=message.chat.id, action="upload_video")
            file_info = await bot.get_file(message.video.file_id)
            ...
            await message.reply("Please send a MOV video file." if user_language == "en" else "Пожалуйста, пришлите видеофайл в формате MOV.")
    except Exception as e:
        await message.reply("An error occurred while converting the video." if user_language == "en" else "Во время конвертации видео произошла ошибка.")
        report = open("report.txt", "a+")
        report.write("\n[" + str(datetime.now(timezone.utc)) + "] Server WARNING error: " + str(e))
        report.close()
...
async def convert_webm_to_mp4_command(message: types.Message):
    await message.reply('Send a webm video file.' if user_language == "en" else "Отправьте видеофайл webm.")
...
async def handle_convert_webm_to_mp4(message: types.Message, state: FSMContext):
    try:
        if message.video and message.video.mime_type == "video/webm":
            ...
            await message.reply("Please send a WebM video file." if user_language == "en" else "Пожалуйста, пришлите видеофайл в формате WebM.")
    except Exception as e:
        await message.reply("An error occurred while converting the video." if user_language == "en" else "Во время конвертации видео произошла ошибка.")
        report = open("report.txt", "a+")
        report.write("\n[" + str(datetime.now(timezone.utc)) + "] Server WARNING error: " + str(e))
...
async def convert_webp_to_jpg_command(message: types.Message):
    await message.reply('Send a WebP image file.' if user_language == "en" else "Отправьте картинку WebP.")
...
async def handle_convert_webp_to_jpg(message: types.Message, state: FSMContext):
    try:
        if message.photo:
            await message.reply("Converting image... \nThis may take a few minutes." if user_language == "en" else "Преобразование картинки... \nЭто может занять несколько минут.")
            await bot.send_chat_action(chat_id=message.chat.id, action="upload_photo")
            file_info = await bot.get_file(message.photo[-1].file_id)
            ...
            await message.reply("Please send a WebP image." if user_language == "en" else "Пожалуйста, пришлите картинку в формате WebP.")
    except Exception as e:
        await message.reply("An error occurred while converting the image." if user_language == "en" else "Во время конвертации картинки произошла ошибка.")
        report = open("report.txt", "a+")
        report.write("\n[" + str(datetime.now(timezone.utc)) + "] Server WARNING error: " + str(e))
        report.close()
    await state.finish()
...
async def convert_ico_to_jpg_command(message: types.Message):
    await message.reply('Send an ICO image file.' if user_language == "en" else "Отправьте картинку ICO.")
...
async def handle_convert_ico_to_jpg(message: types.Message, state: FSMContext):
    try:
        if message.document and message.document.mime_type == 'image/x-icon':
            ...
            await message.reply("Please send an ICO image." if user_language == "en" else "Пожалуйста, пришлите картинку в формате ICO.")
    except Exception as e:
        await message.reply("An error occurred while converting the image." if user_language == "en" else "Во время конвертации картинки произошла ошибка.")
        report = open("report.txt", "a+")
        report.write("\n[" + str(datetime.now(timezone.utc)) + "] Server WARNING error: " + str(e))
...
async def image_text_recognition_command(message: types.Message):
    await message.reply('Send a PNG or JPEG image file.' if user_language == "en" else "Отправьте картинку PNG или JPEG.")
...
async def handle_image_text_recognition(message: types.Message, state: FSMContext):
    try:
        if message.photo:
            ...
            await message.reply('Please send a PNG or JPEG image file.' if user_language == "en" else "Пожалуйста, пришлите картинку в формате PNG или JPEG.")
            return

        await message.reply("Performing OCR... \nThis may take a few minutes." if user_language == "en" else "Выполнение оптического распознавания символов... \nЭто может занять несколько минут.")
        await bot.send_chat_action(chat_id=message.chat.id, action="typing")
        ...
        await message.reply('OCR complete.' if user_language == "en" else "Оптического распознавание символов завершено.")
    except Exception as e:
        await message.reply("An error occurred while performing OCR." if user_language == "en" else "Произошла ошибка при выполнении Выполнение оптического распознавания символов")
        report = open("report.txt", "a+")
        report.write("\n[" + str(datetime.now(timezone.utc)) + "] Server WARNING error: " + str(e))
        report.close()
...
async def password_generator_command(message: types.Message):
    await message.reply('Enter password length:' if user_language == "en" else "Введите длину пароля:")
...
async def handle_password_generator(message: types.Message, state: FSMContext):
    try:
        ...
        password = ''
        await message.reply("Password generation... \nThis may take a few minutes." if user_language == "en" else "Генерация пароля... \nЭто может занять несколько минут.")
        await bot.send_chat_action(chat_id=message.chat.id, action="typing")
        ...
        await message.reply(message, "An error occurred while password generation." if user_language == "en" else "Во время генерации пароля произошла ошибка.")
        report = open("report.txt", "a+")
        report.write("\n[" + str(datetime.now(timezone.utc)) + "] Server WARNING error: " + str(e))
        report.close()
    await state.finish()
...
async def text_to_audio_command(message: types.Message):
    await message.reply('Enter the text you want to convert to audio:' if user_language == "en" else "Введите текст, который вы хотите преобразовать в аудио:")
...
async def handle_text_to_audio(message: types.Message, state: FSMContext):
    try:
        await message.reply("Converting text to audio... \nThis may take a few minutes." if user_language == "en" else "Преобразование текста в аудио... \nЭто может занять несколько минут.")
        ...
        await message.reply("An error occurred while processing." if user_language == "en" else "Произошла ошибка при выполнении.")
        report = open("report.txt", "a+")
        report.write("\n[" + str(datetime.now(timezone.utc)) + "] Server WARNING error: " + str(e))
        report.close()
    await state.finish()
...
async def hash_calculator(message: types.Message):
    await message.reply('Select hashing method:' if user_language == "en" else "Выберите метод хеширования", reply_markup=inlineKeyboardHash)
...
async def handle_hash_calculator(message: types.Message, state: FSMContext):
    try:
        await bot.send_message(message.chat.id, "File Processing..." if user_language == "en" else "Обработка файла")
        ...
        await message.reply("Calculating complete." if user_language == "en" else "Расчет завершен.")
...
async def hash_calculator(message: types.Message):
    await message.reply('Select searching language:' if user_language == "en" else "Выберите язык поиска", reply_markup=inlineKeyboardWiki)
...
async def hash_calculator(message: types.Message, state: FSMContext):
    try:
        await bot.send_message(message.chat.id, "Searching..." if user_language == "en" else "Поиск...")
        ...
        if page.exists():
            ...
            await bot.send_message(message.chat.id, "Searching successfull." if user_language == "en" else "Поиск успешен.")
        else:
            await bot.send_message(message.chat.id, "Page not found." if user_language == "en" else "Страница не найдена.")
    except Exception as e:
        await message.reply("An error occurred while processing." if user_language == "en" else "Произошла ошибка при выполнении.")
        report = open("report.txt", "a+")
        report.write("\n[" + str(datetime.now(timezone.utc)) + "] Server WARNING error: " + str(e))
...
async def hash_calculator(message: types.Message):
    await message.reply('Select currency:' if user_language == "en" else "Выберите валюту.", reply_markup=inlineKeyboardCurrency)

async def show_currency_values(message: types.Message):
    currency_values = await get_currency_values(message)
    if currency_values:
        response = 'Current currency values:\n' if user_language == "en" else "Текущие значения валют:\n"
        for currency, value in currency_values.items():
            response += f'{currency}: {value}\n'
    else:
        response = 'Failed to get actual currency values.' if user_language == "en" else "Не удалось получить актуальные значения валюты."
...
async def get_currency_values(message: types.Message):
    try:
        ...
        await message.reply("Error getting currency values." if user_language == "en" else "Ошибка получения значений валюты.")
        report = open("report.txt", "a+")
        report.write("\n[" + str(datetime.now(timezone.utc)) + "] Server WARNING error: " + str(e))
        report.close()  
    return None
